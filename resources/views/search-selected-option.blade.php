...
>
    <span class="{{ $styles['searchSelectedOptionTitle'] }}">
        {{ data_get($value, 'description', 'Override selectedOption() with keyed array (value, description) for meaningful description') }}
    </span>

    <span
        type="button"
        wire:click.prevent="removeValue('{{ $value['value'] }}')"
    >
